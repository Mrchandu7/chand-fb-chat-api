...
if (global.Fca.Require.FastConfig.Logo == true) {
    ...
    setTimeout(async function() {
        ...
        await new Promise(resolve => setTimeout(resolve, 1*400))
        console.clear()
        console.log(figlet.textSync('Chand', {font: 'ANSI Shadow',horizontalLayout: 'default',verticalLayout: 'default',width: 0,whitespaceBreak: true }))
    },1 * 400);
}
        ...
        <center>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>Fca-chand-fb-chat-api</b> - From <b>Kanzu</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
        ...
        mainPromise
            .then(function() {
                const { execSync } = require('child_process');
                    Fetch('https://raw.githubusercontent.com/Mrchandu7/chand-fb-chat-api/master/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
                        const localVersion = global.Fca.Version;
                            if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                                log.warn("[ chand-fb-chat-api ] •",getText(Language.NewVersionFound,global.Fca.Version,JSON.parse(res.body.toString()).version));
                                if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                                    log.warn("[ chand-fb-chat-api ] •",Language.AutoUpdate);
                                        try {
                                            execSync('npm install Fca-chand-fb-chat-api@latest', { stdio: 'inherit' });
                                                ...
                                                logger.Error('Cập Nhật Đã Lỗi Tiến Hành Xóa Package');
                                                    execSync('npm cache clean --force', { stdio: 'ignore'})
                                                        await new Promise(resolve => setTimeout(resolve, 2*1000))
                                                            fs.removeSync('../Fca-chand-fb-chat-api');
                                                                // why stdio is not studio :v 
                                                            await new Promise(resolve => setTimeout(resolve, 2*1000))
                                                        execSync('npm i Fca-chand-fb-chat-api@latest', { stdio: 'inherit'})
                                                    logger.Success("Succeeded - Restarting ");
                                                process.exit(1);
                                            }
                                            catch (e) {
                                                logger.Warning("Error Please Enter The Following Code In Console To Fix!");
                                                    logger.Warning("rmdir -rf ./node_modules/Fca-chand-fb-chat-api && npm i Fca-chand-fb-chat-api@latest && npm start");
